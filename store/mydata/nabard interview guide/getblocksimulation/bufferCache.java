...
class bufferCache
{
	...
	final freeListEmptyEvent f = new freeListEmptyEvent(); //constructor of this class is called when class is created.
    ...
    bufferHeader getBlock(int blockNumber, String threadName) throws InterruptedException
    {
    	...
    	while(true)
    	{
    		if(presentInHashQueue(blockNumber))
    		{
    			...
    			buffer = getHashQueueReference(blockNumber); //to get the object of buffer.
    		...
    		else 
    		{
    			...
    			if(f.freeListEmpty == true)
    			{
    				synchronized(f)
    				{
    					while(f.freeListEmpty == true)
    					{
                            ...
                            System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
    						f.wait(); //wait and notify can be called on any object.
    					}
    				}
    			}
                ...
                bufferHeader tempNode = takeOutHashQueueBuffer(buffer.blockNumber);     //remove from old hash queue , how.
    ...
    void intializeFreeList() throws InterruptedException
    {
        ...
        while(freeListSize!=maxFreeListSize) //freeListSize=0 just to traverse the freelist and assign each buffer a block number.
        {
